...
import img from "../../../public/assets/images/dsm (10).png";

import { useRouter } from "next/navigation";

import { MapPin, Camera, Mountain, Waves } from "lucide-react";

import { getRelatedRoutes } from "../../../components/utils/destinationUtils";
import Image from "next/image";

// Import the actual route data
const routeData = {
...
const createDestinationSlug = (name: string): string => {
  return name
    .toLowerCase()
    .replace(/[^a-z0-9\s-]/g, "")
    .replace(/\s+/g, "-")
...
const DestinationsPage = () => {
  ...
  // Updated handler to go to destination overview OR single route
  const handleDestinationClick = (destination) => {
    const destinationSlug = createDestinationSlug(destination.name);
    // Check if this destination has only one related route
    const relatedRoutes = getRelatedRoutes(destination.name, routeData);
    if (relatedRoutes.length === 1) {
      // If only one route, go directly to that route
      const [routeKey] = relatedRoutes[0];
      router.push(`/routes/${routeKey}`);
    } else {
      // If multiple routes, go to destination overview
    ...
    <div className="min-h-screen bg-[#fff9f5]">
      ...
      <div className="text-center py-16 px-6">
        ...
        <p className="text-xl text-gray-600 max-w-4xl mx-auto leading-relaxed">
          From the endless plains of Serengeti to the pristine beaches of our
          coastal gems. Discover unforgettable adventures across our handpicked
          destinations.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {destinations.map((destination) => {
            ...
            const config =
              ...
              categoryConfig["Northern Circuit"];
              ...
              >
                ...
                <div className="relative h-48 md:h-80  overflow-hidden">
                  ...
                  >
                    {destination.images.map((image, index) => (
                      <div
                        ...
                        className="w-full cursor-pointer h-full flex-shrink-0 rounded-lg relative overflow-hidden"
                      >
                        <Image
                  ...
                  <div className="absolute top-3 left-3">
                    <span
                      className={`${config.bgColor} ${config.textColor} px-3 py-1 rounded-full text-xs font-medium backdrop-blur-sm`}
                    >
                    ...
                    {destination.images.map((_, index) => (
                      <button
                        ...
                        className={`w-2 h-2 rounded-full transition-all duration-300 ${
                          index === currentSlide
                            ? `${config.color.replace("bg-", "bg-")} scale-125`
                ...
                <div className="p-5 pl-0">
                  <h3
                    className="font-semibold text-gray-800 mb-2 group-hover:text-[#ac8833] transition-colors cursor-pointer text-lg"
                    onClick={() => handleDestinationClick(destination)}
                  >
                    {destination.name}
                  </h3>
                  <p className="text-sm text-gray-600 mb-4">
                    Multiple safari experiences available
                  </p>
                </div>
              </div>
            );
